...
void runSnake() {//...............................run function
  for (int i=size;i>0;i--) {x[i]=x[i-1]; y[i]=y[i-1];}    

  x[0]=x[0]+dirX;
  y[0]=y[0]+dirY;

  if(x[0]==foodX && y[0]==foodY) {size++; getFood();  period=period-1;}
  ...
  for(int i=0;i<10;i++)
  {
    ...
    for(int j=0;j<5;j++)
    {
      if(bulX[i]>enemyX[j] && bulX[i]<enemyX[j]+16 && bulY[i]<enemyY[j]+16 && bulY[i]>enemyY[j]) //colision buton with enemy
      {
        ...
        score++;
        enemyHe[j]--;
        if(enemyHe[j]==0) {score=score+5; enemyX[j]=(random(0,6)*20)+1; enemyY[j]=(random(10,240)*-1); enemyHe[j]=2;}
      }
    }  
...
void loop() {
  ...
  if(game==1)
  { 
    if(millis()>currentTime+period) {runSnake(); currentTime=millis();} 
